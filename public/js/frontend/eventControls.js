define('frontEventControl',
	['jquery', 'utils', 'domReady', 'datetimepicker'],
	function($, utils, datetimepicker) {

		function frontEventControl($, utils, datetimepicker) 
		{
			var self = this;
			...
			self.__presetDate = function(elem, txt, type)
			{
				elem.datetimepicker({ pickTime: false, 
									  startDate: utils.dateFormat('%d/%m/%Y') });
				if (txt.val() == '') {
		...
		return new frontEventControl($, utils, datetimepicker);
	}
);
